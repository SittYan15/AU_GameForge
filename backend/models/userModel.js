...
function toUser(row) {
    ...
    return {
        ...
        tutorialCompleted: row.tutorial_completed === true,
...
export async function findUserByUsername(username) {
    const result = await pool.query(
        `SELECT id, username, password_hash, player_name, points, email, google_sub,
                profile_picture_url, avatar_key, bio, tutorial_completed, active_session_id, active_session_expires_at, created_at, updated_at
         FROM users
         WHERE username = $1`,
        [username]
    );
...
export async function upgradeGuestToPasswordUser(guestId, username, passwordHash) {
    ...
    try {
        ...
        const guestResult = await client.query(
            `SELECT id, player_name, points, avatar_key, bio, tutorial_completed, converted_to_user_id
        ...
        const userResult = await client.query(
            `INSERT INTO users
                (username, password_hash, player_name, points, avatar_key, bio, tutorial_completed)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             RETURNING id, username, password_hash, player_name, points, email, google_sub,
                       profile_picture_url, avatar_key, bio, tutorial_completed, active_session_id, active_session_expires_at, created_at, updated_at`,
            [username, passwordHash, guest.player_name, guest.points, guest.avatar_key, guest.bio, guest.tutorial_completed]
        );
...
export async function addUserPoints(userId, pointsToAdd) {
    const result = await pool.query(
        `UPDATE users
         ...
         WHERE id = $2
         RETURNING id, username, player_name, points, email, google_sub,
                   profile_picture_url, avatar_key, bio, tutorial_completed, created_at, updated_at`,
        [pointsToAdd, userId]
    );
    return toUser(result.rows[0]);
}
...
export async function updateUserProfile(userId, playerName, avatarKey, bio) {
    const result = await pool.query(
        `UPDATE users
         ...
         WHERE id = $4
         RETURNING id, username, player_name, points, email, google_sub,
                   profile_picture_url, avatar_key, bio, tutorial_completed, active_session_id,
