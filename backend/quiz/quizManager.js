...
if (CAMPUS_QUIZ_QUESTIONS.length < CAMPUS_QUIZ_QUESTIONS_PER_ROUND ||
    ...
    throw new Error("Campus Quiz needs at least 10 questions with unique IDs.");
}
...
function buildPublicQuestion(question, state) {
    ...
    return {
        ...
        points: CAMPUS_QUIZ_SCORE_PER_CORRECT,
...
async function finishRound(io) {
    ...
    // v5.0 scoring: every correct answer is worth 10 points.
    // A player keeps the points earned before elimination; persistence still
    // happens only once, at the end of the match.
    for (const socket of getQuizSockets(io)) {
        const correctCount = Math.max(
            0,
            Math.min(
                totalQuestions,
                Number(socket.data.campusQuizCorrectCount) || 0
            )
        );

        socket.data.campusQuizScore =
            socket.data.campusQuizParticipating
                ? Math.min(
                    CAMPUS_QUIZ_MAX_SCORE,
                    correctCount * CAMPUS_QUIZ_SCORE_PER_CORRECT
                )
                : 0;
    }
    ...
    const results = await Promise.all(getQuizSockets(io).map(async socket => {
        ...
        const completedRound = Boolean(
            participated &&
            !socket.data.campusQuizEliminated &&
            socket.data.campusQuizAnsweredQuestions?.size === totalQuestions
        );
        const eligible = participated && score > 0;
        ...
        return { socket, payload: {
            roundId, participated, completed: completedRound, score, correctCount, wrongCount,
...
function evaluateQuestion(io, roundId = quizState.roundId, questionId = quizState.currentQuestion?.id) {
    ...
    for (const socket of activeSockets) {
        ...
        if (correct) {
            ...
            socket.data.campusQuizScore = Math.min(
                CAMPUS_QUIZ_MAX_SCORE,
                socket.data.campusQuizCorrectCount * CAMPUS_QUIZ_SCORE_PER_CORRECT
            );
        } else {
        ...
        socket.emit("campusQuiz:lifeResult", {
            ...
            pointsEarned: correct ? CAMPUS_QUIZ_SCORE_PER_CORRECT : 0,
    ...
    socket.on("campusQuiz:join", async () => {
        ...
        try {
            ...
            if (socket.data.inPropHunt) {
                socket.emit("campusQuiz:error", "Leave Campus Prop Hunt before starting Campus Quiz.");
                return;
            }
