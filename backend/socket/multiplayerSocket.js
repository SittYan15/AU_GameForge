...
import {
    registerPropHuntSocket,
    validatePropHuntMovement
} from "../propHunt/propHuntManager.js";
...
// Return to the campus immediately beside the RLGL playground,
// but far enough from the portal to avoid instant re-entry.
const RLGL_RETURN_POSITION = Object.freeze({
    x: -574.00,
    y: 0.30,
    z: -116.79
});
...
function emitRlglPlayerStatus(io) {
    const rows =
        getRlglSockets(io)
            .map((socket) => {
                const player =
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("player:move", (payload = {}) => {
            ...
            if (socket.data.inPropHunt) {
                const validation = validatePropHuntMovement(
                    socket,
                    player,
                    payload.position
                );
        ...
        socket.on("rlgl:join", async () => {
            ...
            if (socket.data.inPropHunt) {
                socket.emit("rlgl:error", "Exit Campus Prop Hunt before entering Red Light, Green Light.");
                return;
            }
